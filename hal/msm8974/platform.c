...
void platform_add_backend_name(char *mixer_path, snd_device_t snd_device)
{
    if (snd_device == SND_DEVICE_IN_BT_SCO_MIC)
        ...
        strlcat(mixer_path, " voice-speaker", MIXER_PATH_LENGTH);
    else if (snd_device == SND_DEVICE_OUT_SPEAKER_AND_ANC_HEADSET)
        strlcat(mixer_path, " speaker-and-anc-headphones", MIXER_PATH_LENGTH);
    else if (snd_device == SND_DEVICE_OUT_SPEAKER_PROTECTED)
        strlcat(mixer_path, " speaker-protected", MIXER_PATH_LENGTH);
    else if (snd_device == SND_DEVICE_OUT_VOIP_SPEAKER)
        strlcat(mixer_path, " voip-speaker-comm", MIXER_PATH_LENGTH);
}
