...
status_t AudioHardware::AudioStreamOutDirect::standby()
{
    ...
    ALOGV(" AudioStreamOutDirect::standby mHardware->mNumVoipStreams = %d mHardware->mVoipFd = %d\n", mHardware->mNumVoipStreams, mHardware->mVoipFd);
    if (mHardware->mVoipFd >= 0 && (mHardware->mNumVoipStreams == 1)) {
#ifdef LEGACY_QCOM_VOICE
        msm_end_voice();
#else
